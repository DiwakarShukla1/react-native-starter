...
export default class StepIndicator extends PureComponent {
  ...
  renderStepIndicator = () => {
    ...
    for(let position = 0 ; position < stepCount ; position++) {
      steps.push(
        <TouchableWithoutFeedback disabled={true}  key={position} onPress={() => this.stepPressed(position)}>
    ...
    renderStepLabels = () => {
      ...
      var labelViews = labels.map((label,index) => {
        ...
        return (
          <TouchableWithoutFeedback disabled={true} style={styles.stepLabelItem} key={index} onPress={() => this.stepPressed(index)}>
